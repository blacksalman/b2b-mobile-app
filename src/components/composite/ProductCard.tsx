...
interface GridProps extends ProductCardHandlers {
  variant: 'grid';
  product: RailProduct & { badge?: string; hasOffer: boolean; noOffer: boolean };
...
function CartControls({
  ...
  return (
    <Pressable onPress={onAdd} style={[styles.addButton, { height, marginTop: 10 }]}>
...
function GridCard({ product: p, dense, onOpen, onAdd, onInc, onDec, onGoCart, onLogin }: GridProps) {
  ...
  return (
    <View style={styles.gridCard}>
      ...
      <View style={styles.body}>
        {dense && <MarginChip margin={p.margin} />}
        <Pressable onPress={onOpen}>
          <Text
            style={[styles.name, { fontSize: dense ? 13 : 14, minHeight: dense ? 34 : 36, marginTop: dense ? 9 : 0 }]}
            numberOfLines={2}
          >
            {p.name}
          </Text>
        ...
        {p.gated ? (
          ...
          p.hasOffer ? (
            <>
              ...
              <CartControls inCart={p.inCart} cartQty={p.cartQty} onAdd={onAdd} onInc={onInc} onDec={onDec} onGoCart={onGoCart} height={controlHeight} />
            </>
          ) : (
            <>
